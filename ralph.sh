...
# Beads database location - use the centralized planning database
# bd commands need to run from this directory to use the correct database
BEADS_DIR="$HOME/.beads-planning"
...
Notes:
  ...
  - Issues are read from ~/.beads-planning (centralized database)
EOF
    exit 0
}
...
validate_epic() {
    if [[ -n "$EPIC_ID" ]]; then
        if ! (cd "$BEADS_DIR" && bd show "$EPIC_ID") &>/dev/null; then
...
# Build the bd ready command with filters (runs from BEADS_DIR)
build_ready_cmd() {
    local cmd="cd $BEADS_DIR && bd ready"
...
count_remaining() {
    local ready_cmd="$1"
    local count
    count=$(bash -c "$ready_cmd" 2>/dev/null | grep -c '^\([0-9]\|○\|◐\)' || true)
    echo "${count:-0}"
}
...
main() {
    ...
    while [[ $iteration -lt $MAX_ITERATIONS ]]; do
        ...
        bash -c "$ready_cmd" 2>/dev/null | head -5
        echo ""
