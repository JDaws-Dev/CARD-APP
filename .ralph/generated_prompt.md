...
**IMPORTANT:** All `bd` commands must run from `~/.beads-planning` directory to access the correct database.

For all bd commands, either:
1. Run `cd ~/.beads-planning && bd <command>`, OR
2. Prefix with: `cd ~/.beads-planning &&`

Example: `cd ~/.beads-planning && bd show CD-abc`
